...
function AssistantAppointments() {
  ...
  const loadAppointments = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/assistant/appointments`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = selectedAppointment 
        ? `${API_BASE_URL}/assistant/appointments/${selectedAppointment.id}`
        : `${API_BASE_URL}/assistant/appointments`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${API_BASE_URL}/assistant/appointments/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
